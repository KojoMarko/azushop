...
interface ProductReviewsProps {
  reviews: Review[]
  // Removed the unused productId prop from the interface
}
...
export function ProductReviews({ reviews }: ProductReviewsProps) {
  ...
  const renderStars = (rating: number) => {
    return Array(5)
      .fill(0)
      .map((_, i) => (
        <svg key={`star-${i}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={i < Math.floor(rating) ? "#FFD700" : "#E5E7EB"} className="w-4 h-4">
          <path
            fillRule="evenodd"
      ...
      <div className="border-b flex space-x-8 mb-6">
        <button
          className={`pb-2 font-medium ${
            activeTab === "related" ? "border-b-2 border-blue-600 text-blue-600" : "text-gray-500 hover:text-gray-700"
          }`}
        ...
        <button
          className={`pb-2 font-medium ${
            activeTab === "write" ? "border-b-2 border-blue-600 text-blue-600" : "text-gray-500 hover:text-gray-700"
          }`}
